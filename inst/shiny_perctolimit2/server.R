...
shinyServer(function(input, output, session) {
    ...
    uni.bkevenf <- reactive({
      ...
      uni.bkeven$"Value at indifference point" <- round(uni.bkeven$"break",2)
      uni.bkeven
    })
    ...
    output$uni_plot_variable<- renderUI({
      ...
      radioButtonsTable("uni_plot_variable_selected",
                        "Univariate break-even values", uni.bkeven[,c("% distance from best guess","% distance from bound","Value at indifference point")],
